...
import DeveloperGameCardContextMenu from "./DeveloperGameCardContextMenu";
...
const DeveloperStoreGameCard = ({ game, isComing }: IPropsStoreGameCard) => {
  ...
  return (
    <>
        ...
        <div className="store-game-card2">
          <TooltipComponent placement="bottom" text={game?.title}>
            <Box
              ...
              onClick={() => {
                navigate(`/developer-store/${game?._id}`);
              }}
            >
              {isComing && (
                <Box
      ...
      <DeveloperGameCardContextMenu contextMenuPosition={contextMenuPosition} view={showContextMenu} setView={setShowContextMenu} game={game} />
    </>
  );
};

export default DeveloperStoreGameCard;
