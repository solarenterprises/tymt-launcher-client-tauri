  ...
  return (
    game?.projectMeta?.system_requirements && (
      <Box marginTop={"12px"} borderBottom={"1px solid rgba(255, 255, 255, 0.10)"} paddingBottom={"16px"}>
        ...
        <Stack mt={"16px"}>
          {game?.projectMeta?.system_requirements?.cpu && (
            <Box className={"fs-14-regular white"}>{`• PROCESSOR: ${game.projectMeta.system_requirements.cpu}`}</Box>
          )}
          {game?.projectMeta?.system_requirements?.gpu && (
            <Box className={"fs-14-regular white"}>{`• GRAPHICS: ${game.projectMeta.system_requirements.gpu}`}</Box>
          )}
          ...
          {game?.projectMeta?.system_requirements?.disk && (
            <Box className={"fs-14-regular white"}>{`• STORAGE: ${game.projectMeta.system_requirements.disk}`}</Box>
          )}
        </Stack>
      </Box>
    )
