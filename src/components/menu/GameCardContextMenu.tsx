...
import { addOneLibraryList, delOneLibraryList, getLibraryList } from "../../store/LibraryListSlice";
...
const GameCardContextMenu = ({ view, setView, contextMenuPosition, game }: IPropsUserListItemContextMenu) => {
  ...
  const isInLibrary = useMemo(() => libraryListStore?.games?.some((one) => one?._id === game?._id), [libraryListStore]);

  const handleOnClose = () => {
    setView(false);
  };

  const handleAddToLibrary = (currentGame: IGame) => {
    dispatch(addOneLibraryList(currentGame));
    setView(false);
  };

  const handleRemoveFromLibrary = (currentGame: IGame) => {
    dispatch(delOneLibraryList(currentGame));
    setView(false);
  };
  ...
  const handleRemove = async (currentGame: IGame) => {
    try {
      setView(false);
    ...
    <>
      <Modal open={view} onClose={handleOnClose}>
        <Fade in={view}>
          ...
          >
            {game?.projectMeta?.type === "browser" && isInLibrary && (
              <>
            ...
            {game?.projectMeta?.type === "native" && installed && (
              <>
                ...
                <Box className={"fs-16 white context_menu_bottom"} textAlign={"left"} sx={{ backdropFilter: "blur(10px)" }} onClick={() => handleRemove(game)}>
                  {t("ga-44_remove")}
                </Box>
              </>
            )}
            {game?.projectMeta?.type === "native" && !installed && (
              <Box className={"fs-16 white context_menu_single"} textAlign={"left"} sx={{ backdropFilter: "blur(10px)" }} onClick={() => handleDownload(game)}>
                {t("lib-3_download")}
              </Box>
            )}
          </Box>
